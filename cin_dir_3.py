...
# Eje rotacion es Z
# Z es perpendicular a la rotación
# Si es posible, copiar el sistema
...
#        1    2    23       3    4          51      52   
d  = [p[0],   0,    0,   p[2],   0,          0,     0]
th = [   0, p[1],  90,      0,   90+p[3],     90-p[4], 90+p[4]]
a  = [   0,   5,    0,      0,   0,          2,     2]
al = [   0,   0,   90,      0,   90,         0,     0]
...
T2_23=matriz_T(d[2],th[2],a[2],al[2])
T23_3=matriz_T(d[3],th[3],a[3],al[3])
T23=np.dot(T2_23,T23_3)
T34=matriz_T(d[4],th[4],a[4],al[4])
TEF=matriz_T(0, 90, 2, 0)
T4_51=matriz_T(d[5],th[5],a[5],al[5])
T4_52=matriz_T(d[6],th[6],a[6],al[6])
T02=np.dot(T01,T12)
T03=np.dot(T01,np.dot(T12,T23))
T04=np.dot(T01,np.dot(T12,np.dot(T23,T34)))
T051=np.dot(T01,np.dot(T12,np.dot(T23,np.dot(T34,T4_51))))
T052=np.dot(T01,np.dot(T12,np.dot(T23,np.dot(T34,T4_52))))
T0EF = np.dot(T01, np.dot(T12, np.dot(T23, np.dot(T34, TEF))))
...
o51_0 =np.dot(T051, o51).tolist()
o52_0 =np.dot(T052, o52).tolist()
ef = np.dot(T0EF, ef).tolist()

# Mostrar resultado de la cinemática directa
# Los puntos de la pinza forman un único punto -> Incluir como una lista de listas. Se debe meter después el punto efector EF.
muestra_origenes([o00,o10,o20,o30,o40, o51_0, o52_0])
muestra_robot   ([o00,o10,o20,o30,o40, [[o51_0], [o52_0]]], ef)
input()